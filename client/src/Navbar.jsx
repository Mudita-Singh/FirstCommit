 ...
 */
export default function Navbar({ isHome, onLogoClick, user, authLoading, onLogout, hasActiveFile, analysisData }) {
  const getLoginRedirectUrl = () => {
    let redirectPath = window.location.pathname + window.location.search;
    if (analysisData && analysisData.owner && analysisData.repo) {
      redirectPath = `/workspace?repo=${encodeURIComponent(`https://github.com/${analysisData.owner}/${analysisData.repo}`)}`;
    }
    const baseUrl = import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:5000' : '');
    return `${baseUrl}/api/auth/github?redirect=${encodeURIComponent(redirectPath)}`;
  };
